...
目标：
- 首要优化目的：提升状态型后台链路在并发、异常和宿主兼容性下的稳定性、可预测性与可诊断性。
- 优先处理高频状态读写与头像回滚链路，再处理 Bot API 防腐层和模型校验，不在当前阶段继续扩张新的抽象层。
...
- [ ] 优先级 1：保留并验证 SQLite 的 WAL、`busy_timeout` 与初始化迁移语义，避免异步化后引入新的 `database is locked` 高频异常。
- [ ] 优先级 2：头像回滚兼容性与诊断增强。继续排查 `set_group_portrait` 对本地绝对路径与 `file:///` 路径的接受差异，避免轮询回滚路径出现挂起、静默失败或回滚后校验失真。
- [ ] 优先级 2：补强头像回滚链路日志上下文，至少区分基线路径缺失、API 调用失败、回滚后哈希校验失败三类原因，便于线上排障。
...
- 头像回滚失败时，日志能够明确区分路径格式问题、宿主 API 报错与回滚后校验失败，不再只暴露笼统失败信息。
